...
from student import DBKD_Classification_Model, DBKD_MultiChoice_Model
...
class ModelArguments:
    ...
    sigma: Optional[float] = field(
        default=1, metadata={"help": "Sigma value"}
    )
...
def main():
    ...
    if "race" in data_args.task_name or data_args.task_name == "dream":
        model = DBKD_MultiChoice_Model.from_pretrained(
        ...
        model = DBKD_Classification_Model.from_pretrained(
            model_args.model_name_or_path,
            from_tf=bool(".ckpt" in model_args.model_name_or_path),
            config=config,
            cache_dir=model_args.cache_dir,
    ...
    def preprocess_function(examples):
        ...
        if "race" in data_args.task_name:
            ...
            result["label"] = [options[i].index(ans) for i, ans in enumerate(examples["answer"])]
        else:
            args = (
    ...
    if training_args.do_train:
        ...
        if model_args.strategy == "hard_aug":
            label_name = "label" if "label" in train_dataset.column_names else "labels"
            label_column = train_dataset[label_name]
            aug_times = 10
            label_column = [lb for lb in label_column for i in range(aug_times)]
            train_dataset = torch.load(os.path.join(model_args.soft_label_dir, "train_dataset.pt"))
            train_dataset = train_dataset.remove_columns("idx")
            train_dataset = train_dataset.remove_columns(label_name)
            idx_column = [i for i in range(len(train_dataset))]
            train_dataset = train_dataset.add_column("idx", idx_column)
            train_dataset = train_dataset.add_column(label_name, label_column)
    ...
    if training_args.do_predict and ("race" in data_args.task_name or "dream" in data_args.task_name):
        if "test" in datasets:
            ...
            trainer.log_metrics("%s_test" % data_args.task_name, metrics)
            trainer.save_metrics("%s_test" % data_args.task_name, metrics)
        ...
        for test_dataset, task in zip(test_datasets, tasks):
            ...
            output_test_file = os.path.join(training_args.output_dir, f"test_results_{task}.tsv")
